...
void DRV832x_initSPIInterface(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, SPI_HandleTypeDef* hspi, GPIO_TypeDef* Enable_GPIOx, uint16_t Enable_GPIO_Pin){
	DRV832x_enable(Enable_GPIOx,Enable_GPIO_Pin);
	s_drv832xSPI.PORT_GPIOx = GPIOx;
	s_drv832xSPI.CS_GPIO_Pin = GPIO_Pin;
	s_drv832xSPI.pSPI_Handle = hspi;
...
void DRV832x_faults(void)
{
    ...
    if(val1 & (1<<10)){LCD_ShowString(10,90,"\n\rFAULT\n\r",WHITE);}

    if(val1 & (1<<9)){LCD_ShowString(10,90,"VDS_OCP\n\r",WHITE);}
    if(val1 & (1<<8)){LCD_ShowString(10,90,"GDF\n\r",WHITE);}
    if(val1 & (1<<7)){LCD_ShowString(10,90,"UVLO\n\r",WHITE);}
    if(val1 & (1<<6)){LCD_ShowString(10,90,"OTSD\n\r",WHITE);}
    if(val1 & (1<<5)){LCD_ShowString(10,90,"VDS_HA\n\r",WHITE);}
    if(val1 & (1<<4)){LCD_ShowString(10,90,"VDS_LA\n\r",WHITE);}
    if(val1 & (1<<3)){LCD_ShowString(10,90,"VDS_HB\n\r",WHITE);}
    if(val1 & (1<<2)){LCD_ShowString(10,90,"VDS_LB\n\r",WHITE);}
    if(val1 & (1<<1)){LCD_ShowString(10,90,"VDS_HC\n\r",WHITE);}
    if(val1 & (1)){LCD_ShowString(10,90,"VDS_LC\n\r",WHITE);}

    if(val2 & (1<<10)){LCD_ShowString(10,110,"SA_OC\n\r",WHITE);}
    if(val2 & (1<<9)){LCD_ShowString(10,110,"SB_OC\n\r",WHITE);}
    if(val2 & (1<<8)){LCD_ShowString(10,110,"SC_OC\n\r",WHITE);}
    if(val2 & (1<<7)){LCD_ShowString(10,110,"OTW\n\r",WHITE);}
    if(val2 & (1<<6)){LCD_ShowString(10,110,"CPUV\n\r",WHITE);}
    if(val2 & (1<<5)){LCD_ShowString(10,110,"VGS_HA\n\r",WHITE);}
    if(val2 & (1<<4)){LCD_ShowString(10,110,"VGS_LA\n\r",WHITE);}
    if(val2 & (1<<3)){LCD_ShowString(10,110,"VGS_HB\n\r",WHITE);}
    if(val2 & (1<<2)){LCD_ShowString(10,110,"VGS_LB\n\r",WHITE);}
    if(val2 & (1<<1)){LCD_ShowString(10,110,"VGS_HC\n\r",WHITE);}
    if(val2 & (1)){LCD_ShowString(10,110,"VGS_LC\n\r",WHITE);}
}